 ...
 *
 * Port of mihomo's transport/snell/v4.go.
 */
...
// v4Conn wraps a TCP-like net.Conn with the Snell v4 AEAD frame format.
// Reader and writer initialize lazily on the first Read/Write — the salt is
// emitted as part of the first written frame and consumed on the first read.
type v4Conn struct {
	net.Conn
	psk []byte
	r   *v4Reader
...
// WritePacketFrame writes b as a single snell frame, used for UDP-over-TCP
// where datagram boundaries must be preserved.
...
// WriteTo / ReadFrom give the standard library copy fast paths something
// to work with.
...
func (r *v4Reader) readFrame() ([]byte, error) {
	...
	if payloadLength > MaxPayloadLength || paddingLength > MaxPayloadLength {
...
func (w *v4Writer) Write(b []byte) (int, error) {
	...
	for written < len(b) {
		...
		if payloadLimit <= 0 || payloadLimit > MaxPayloadLength {
...
// nextPayloadLimit mirrors the reference implementation's frame-size
// ramp-up: the very first frame is small (to make the salt+padding+payload
// fit in a single MTU), then each subsequent frame in a "burst" can be
// larger until it saturates at MaxPayloadLength.
func (w *v4Writer) nextPayloadLimit() uint16 {
	...
	if payloadLimit <= MaxPayloadLength-1 {
		next := int(payloadLimit) + v4FrameSize - 39
		if next > MaxPayloadLength {
			next = MaxPayloadLength
		}
		w.payloadLimit = uint16(next)
	} else {
		w.payloadLimit = MaxPayloadLength
	}
	return payloadLimit
}
...
func (w *v4Writer) writeFrame(payload []byte, paddingLength int) error {
	if len(payload) > MaxPayloadLength || paddingLength > MaxPayloadLength {
...
// swapPadding interleaves the even-indexed bytes of `padding` with those of
// `payloadCipher`. The receiver does the same swap to recover both streams.
...
// makeV4Padding picks padding bytes such that the bit-count ratio of the
// resulting on-the-wire data stays inside a "natural" band — defeats the
// trivial entropy fingerprint of AEAD-only traffic.
...
func makeV4RandomPadding(length int) ([]byte, error) {
	...
	_, err := io.ReadFull(crand.Reader, padding)
	return padding, err
}
...
func randomUnitFloat64() (float64, error) {
	n, err := crand.Int(crand.Reader, big.NewInt(1<<53))
	if err != nil {
		return 0, err
	}
