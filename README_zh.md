...
两个平台都运行**仅 IPv4** 的 fake-IP 路径。AAAA 查询会收到空的 NOERROR 回复，
让解析器回退到 A；随后 snell 服务器使用自己的 DNS 解析该主机名，并通过 v4 建连。
这样可以避开两类失败模式：其一，支持 v6 的主机可能让 v6 DNS 路径完全绕过 fake-IP 层；
其二，仅 v4 的 snell 服务器无法访问 v6 目标。
...
| `SIGTERM`/`SIGINT` 清理 | 删除 nftables 表，移除 priority-1 `ip rule` | 销毁 utun，删除 auto-route，恢复系统 DNS |
...
| AAAA DNS 查询 | 返回空 NOERROR（应用回退到 A）。没有 IPv6 fake-IP 路径 |
| 出站 IPv6 流量 | **不接管** — TUN 仅处理 v4。已经持有真实 v6 地址的应用（例如 IP 字面量或缓存）如果有原生 v6 出口，会绕过 snell 直接连接 |
| 本机进程的 UDP（DNS 以外） | **Linux：不变**（只接管 DNS）。**macOS：丢弃**（TUN 会接住所有 v4 UDP，v2 只处理 DNS） |
...
v2 暂未包含（计划后续加入）：应用 UDP 流量接管（DNS 本身已被接管，但其他 UDP，
例如 WebRTC、QUIC 等，仍然直连）；基于 cgroup-v2 的排除能力（这样 systemd unit
不必改变 `User=` 也可以被豁免）；更完整的 macOS 支持和 Windows 支持。
