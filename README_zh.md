...
> 才建议使用 `alpha` 分支。
>
> Alpha 开发的不可破坏基础是：**OpenSnell 服务端必须始终兼容 Surge 官方客户端**。
> 服务端实验可以增加实现细节、运维 hook 或可选行为，但不能要求修改客户端，
> 不能改变已经公开的 Snell 线路格式，也不能破坏 Surge 官方客户端连接
> OpenSnell 服务端的标准路径。

OpenSnell 是 Snell 代理协议 **v4** 和 **v5** 的 Go 实现，同时提供服务端和客户端。
下文列出的每一条路径都已经与官方 Surge `snell-server v5.0.1` 完成端到端互通验证。

Snell v5 的 UDP/QUIC 代理模式目前只在**服务端**实现；如果你需要为下游应用加速
HTTP/3，请搭配 **Surge** 客户端，或其他支持 v5 的客户端使用。

### 为什么没有 v1 / v2 / v3？

本项目有意放弃了更早的 Snell 协议版本。它们的流分帧格式早于 v4 的
padding/AEAD 重设计，如今在线路上已经很容易被指纹识别。尤其是 v1/v2/v3 的流量特征
已经很难稳定穿越 GFW，因此通常不建议用于新的部署。如果你还有暂时无法下线的
v1/v2 旧环境，同类项目 [open-snell](https://github.com/icpz/open-snell) 及其分支
仍然实现了这些版本；本代码库只专注于当前 Surge `snell-server` 使用的 v4/v5 线路协议。
...
| **TUN：隧道内 IPv6 可达性探测** | — | ✅                                    |
| **TUN：UDP/443 注入 ICMP Unreachable，让 QUIC 快速失败** | — | ✅          |
...
- 让你在 **OpenSnell**（默认，GPLv3，支持全平台）和
  **官方 Surge `snell-server v5.0.1`**（闭源，仅 Linux）之间选择。
- 如果 PSK 留空，会用 `openssl` 生成随机 PSK。
...
通道信息会持久化到 `/etc/snell/.install_meta`，因此之后运行 `update` 时会继续停留在
alpha 通道，不需要重复传入该参数。如果要切回稳定通道，请重新运行一次不带该参数的
`install`。

Alpha 构建在线路兼容性上与稳定版保持同样的保证（CI 会验证其与 Surge
`snell-server v5.0.1` 的完整互通），但上面列出的额外功能尚未进入官方 Snell 规格，
之后可能会被移除或重新设计。生产环境请自行评估风险。
...
; 如果前面还有反向代理，则可设置为 127.0.0.1:<port>。
; 当 `quic = true`（默认）时，服务端还会在同一端口监听 UDP，
; 用于 QUIC 代理模式。因此请确保主机前方的防火墙同时放行
; TCP/<port> 和 UDP/<port>。
listen = 0.0.0.0:2333
...
; apernet/tcp-brutal 内核拥塞控制算法，并按 `brutal-mbps` 固定速率发送。
...
UDP ASSOCIATE），并把所有接收到的请求都通过 snell 服务端建立隧道。
如需使用 QUIC/HTTP-3，请用 Surge 作为前端；本客户端面向已经支持 SOCKS5 的工具，
例如 `curl --socks5-hostname`、浏览器代理设置、应用内 SOCKS5 接口等。
...
; --- 实验性：客户端出站的 tcp-brutal 拥塞控制 ---
; 启用后，每条连接到 snell 服务端的 TCP 出站连接都会切换到 brutal CC。
...
### 实验性：TUN 入站 — 透明接管 TCP + fake-IP DNS（Linux + macOS）

除了 SOCKS5 监听之外，`snell-client` 还可以透明接管机器上**每条新发起的出站
TCP 连接**，并通过 snell 服务器转发。应用侧不需要知道 SOCKS5 的存在，同时**原始
主机名会端到端保留**，由 snell 服务器自行完成干净的 DNS 解析。整体架构与
clash / mihomo / sing-box 的思路一致。
...
fake-IP 层的意义在于：很多用户所在网络的 ISP DNS 会**污染** docker.io、
github.com、googlevideo.com 等域名。如果本机先完成解析，只有一个 IP 被送到
snell 服务器，那么这个 IP 可能已经是错的，snell 也无能为力。使用 fake-IP 后，
snell 服务器会从它自己更干净的上游解析目标；你也可以通过服务端的 `dns = …`
固定这个上游。

#### IPv6 策略

fake-IP 路径**只处理 IPv4**：AAAA 查询会得到空 NOERROR 响应，让解析器回退到 A；
随后由 snell 服务器使用自己的 DNS 解析主机名并以 v4 建连。

仅靠这一点仍然无法覆盖绕过 fake-DNS 的流量，例如 Chrome 的 Secure DNS /
Firefox DoH、缓存了 AAAA 的服务、直接连接 v6 字面量的应用。针对这类情况，TUN
handler 会在启动时以及每 5 分钟运行一次**隧道内 IPv6 可达性探测**：通过 snell
隧道拨号 `[2606:4700:4700::1111]:443`（Cloudflare DNS）。如果探测成功，说明服务器
具备可用的 v6 出口，到达 handler 的真实 v6 目标会被正常转发；如果探测失败，handler
会立即丢弃真实 v6 目标，让 happy-eyeballs 应用在毫秒级回退到 A，而不是卡在内核的
connect 超时上。

`ipv6 = false` 开关（在 `[snell-tun]` 下）会无视探测结果，强制关闭 v6。它适用于
v6 连接故障无法被当前探测目标识别的主机，或者你只是希望行为稳定地限定在 v4。

#### 分平台机制

| 关注点 | Linux | macOS |
| ------ | ----- | ----- |
| DNS 拦截 | `nftables` 将 UDP/TCP :53 DNAT 到 TUN 网关 | `networksetup -setdnsservers` 按网络服务改写 DNS（捕获并在退出时恢复） |
...
| Direct IP / Direct Domain 旁路 | `netipx.IPSet` 写入 `RouteExcludeAddressSet`；通过 `AutoRedirect.UpdateRouteAddressSet()` 实时重新发布 | `/sbin/route add -host/-net … -interface <iface>`；最长前缀匹配优先于 utun auto-route 的半前缀。默认接口在启动时探测一次 |
| 进程异常退出后的残留路由清理 | 不需要（每次启动都会整张删除 nftables 表） | 启动时回放 `/var/run/opensnell-bypass.state`，逐条执行 `route delete`，然后清空文件 |
| QUIC 快速失败 | 对 fake-IP 目标的 UDP/443 注入 ICMP Port Unreachable | 对 TUN 接住的任意目标的 UDP/443 注入 ICMP Port Unreachable |
| `SIGTERM`/`SIGINT` 清理 | 删除 nftables 表，移除 priority-1 `ip rule` | 销毁 utun，删除 auto-route，恢复系统 DNS，撤销 Direct 路由 |

DNS 拦截方式之所以不同，是因为 macOS 的 `mDNSResponder` 使用按网络服务划分的
DNS 作用域，会把查询绑定到 Wi-Fi/Ethernet 源接口并直接从该接口发出；单纯接管默认路由时，
...
| 出站 UDP/443（Web QUIC） | **注入 ICMP Port Unreachable** 给应用，让 QUIC 立即放弃并回退到 TCP，而不是静默丢弃后等待约 10 秒超时 |
...
除了“全部走 snell”之外，你也可以把特定目标标记为 **direct**：由主机自己的路由栈处理，
snell 完全看不到。适合 LAN 网段、公司内网，或在代理后表现异常的第三方服务。

两个粒度，都配置在 `[snell-tun]` 下：

- **`direct-ip`** — 逗号分隔的 CIDR（`10.0.0.0/8, 192.168.0.0/16`）
  或裸 IP（`1.1.1.1, 8.8.8.8`）。静态配置，启动时加载。
- **`direct-domain`** — 逗号分隔的 DNS 后缀列表
  （`internal.example.com, intranet.local`）。fake-IP DNS 服务器看到匹配的查询时，
  会把它原样转发给 `upstream-dns`，而不是合成 fake-IP；随后把回包里的每个
  A/AAAA 地址按记录自带的 TTL 注册进旁路集。每 30 秒会清理一次过期项。

两个开关汇入同一套分平台旁路机制：

| 平台 | 旁路机制 |
| ---- | -------- |
| **Linux** | 加进 `netipx.IPSet`，再调 sing-tun 的 `AutoRedirect.UpdateRouteAddressSet()`，让 nftables 即时重新发布 REDIRECT 排除集 |
| **macOS** | 调用 `route add -host <ip> -interface <默认接口>`，让内核最长前缀匹配优先于 sing-tun auto-route 的半前缀。默认接口启动时通过 `route -n get default` 探测一次 |

Direct Domain DNS 转发器在 Linux 上会给上游 UDP 查询打上 snell-client 的
`OutputMark`（SO_MARK 0x2024），让查询绕过 sing-tun 的 nft DNS 劫持；否则转发器会
绕回我们自己的 fake-IP DNS 服务器，形成回环。

**崩溃后的路由清理（macOS）：** 旁路管理器每安装一条路由，就会把它同步记录到
`/var/run/opensnell-bypass.state`（temp+rename 原子写）。启动时会回放这个状态文件：
先逐条执行 `route delete`，再清空文件，避免上一次被 SIGKILL 的进程在内核路由表里
留下孤立的 /32 记录。Linux 不需要等价机制：sing-tun 的 nftables 表每次启动都会整张删除，
排除集也会随之清空。
...
启动后，`curl https://corp.example.com/internal` 会走主机原生路由直达内网，
而 `curl https://www.cloudflare.com/` 仍然经过 snell 隧道。

#### 排除特定服务（仅 Linux）

如果同一台机器上还运行透明转发器（`realm`、`gost`、`socat` 一类端口转发工具），
通常会希望它们继续直连出站：snell 服务器并不是这个转发器的目标；这个转发器本身才是其他人的目标。
...
; 可以无视探测结果强制关闭 v6，适合探测目标无法准确反映实际连通性的环境，
; 或者你只是希望行为稳定地限定在 v4。
;ipv6 = true
...
; 完全绕过代理，使用主机原本的路由。启动时静态加载。典型用途：LAN
; 网段、公司内网、在代理后表现异常的特定服务。
;direct-ip = 10.0.0.0/8, 192.168.0.0/16, 1.1.1.1
...
- **Fake-IPv6 池**：AAAA 查询目前返回空 NOERROR，让应用回退到 A。
  我们不会分配 fake-IPv6 地址。这意味着仅 v6（没有 A 记录）的目标无法走代理。
  临时解决方式是：如果你有原生 v6 出口，可以把这类域名列入 `direct-domain`；
  否则该域名必须能通过 v4 访问。未来的 v3 会在 v6 探测成功时加入 ULA fake-IPv6 池。

- **Linux IPv6 UDP 接管**：Linux 上只有 IPv4 UDP/443 流量会到达 handler
  （TUN 持有 fake-IPv4 CIDR；nft auto-redirect 只接管 TCP）。真实 v6 UDP
  流量，包括 v6 QUIC，都会走主机原生接口，不会收到 ICMP 回退包。实际上这只在
  “服务器没有 v6 但主机有 v6”时有影响：应用会继续通过主机原生 v6 成功使用 QUIC，
  这也是我们希望看到的结果。

- **macOS 默认接口热切换**：旁路管理器只在启动时探测一次默认物理接口。
  如果 snell-client 运行期间从 Wi-Fi 切到以太网，已经安装的 direct 路由仍会指向旧接口，
  直到重启前都会失效。订阅 sing-tun 的默认接口监视器并在切换时重装路由，仍在待办列表中。

- **非 443 端口的 QUIC**：只有 UDP/443（HTTP/3 的标准端口）会注入
  ICMP unreachable。其他端口上的自定义 QUIC 应用仍然会静默丢弃；对任意 UDP 端口都注入
  ICMP 会破坏游戏 / VoIP / mDNS，代价比 10 秒 QUIC 超时更高。

- **UDP 应用流量代理**：仍未实现（DNS 和 UDP/443 ICMP 注入是目前仅有的 UDP 处理）。
  WebRTC 和其他 UDP 原生应用在 Linux 上直连出站，在 macOS 上被丢弃。本 alpha 暂不处理这部分。

- **基于 cgroup-v2 的排除**（Linux）：目前按 UID 匹配旁路用户。给 systemd unit 加
  `User=` 是现阶段的替代方案。原生 cgroup-v2 路径分类可以让豁免跟随 unit 身份，
  但还没有实现。
...
- 每条流的第一帧都会额外携带一段 `0x100..0x1FF` 字节的 padding，
  其长度会被选择到让 salt+padding+ciphertext 的整体 0/1 比例落在“自然”的范围内；
  后续帧会将最大 payload 从较小的初始值逐步提高到 `MaxPayloadLength`，
  并在空闲 30 秒后重置。这就是 v5 的 **Dynamic Record Sizing** 优化。
